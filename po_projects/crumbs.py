...
site.update({
    ...
    'project-details': ugettext_lazy('{% load i18n %}<small class="subhead">{% trans "Project" %}</small> {{ project.name }}'),
    'project-update': ugettext_lazy('Project update'),
    'project-download': None,
    'catalog-details': ugettext_lazy('{% load i18n %}<small class="subhead">{% trans "Catalog" %}</small> {{ catalog.get_locale_name }}'),
